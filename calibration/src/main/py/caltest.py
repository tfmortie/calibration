...
""" Nonparametric bootstrapping test for general ensemble calibration. Improved (after WUML)
...
"""
def npbetest_improved(P, y, params):
    ...
    for b in range(params["n_resamples"]):
        ...
        P_b = np.stack(P_b)
        # take convex combination of ensemble predictions
        P_bar_b = np.matmul(np.swapaxes(P,1,2),l)
...
def npbetest_alpha_improved(P, y, params):
    if params["optim"] == "neldermead":
        l = np.array([1/P.shape[1]]*P.shape[1])
        bnds = tuple([tuple([0,1]) for _ in range(P.shape[1])])
        cons = ({'type': 'eq', 'fun': constr})
    ...
    # calculate 1-alpha quantile from sampling distribution
    q_alpha = np.quantile(stats,1-(np.array(params["alpha"])))
    decision = list(map(int,minstat>q_alpha))
...
def _npbetest_alpha_improved(P, y, alpha, params):
    params["alpha"] = alpha
    dec, l = npbetest_alpha_improved(P, y, params)
