"""
Data module for calibration experiments
...
class AmazonDataset(Dataset):
    def __init__(self, dim, struct_path, svm_path):
        ...
        f.close()
        # now process data
        self.X = sparse.lil_matrix((n,dim), dtype=np.float)
        self.y = []
        f = open(svm_path)
    ...
    def __getitem__(self, idx):
        inp = self.X[idx].toarray().flatten()
        inp = torch.Tensor(inp)
        return inp, self.y[idx]

""" general amazon dataloaders """
def AmazonDataloaders(args):
    trainval_dataset = AmazonDataset(args.dim, args.datapath+"/hierarchy_full.txt", args.datapath+"/amazon_train_tfidf.svm")
    test_dataset = AmazonDataset(args.dim, args.datapath+"/hierarchy_full.txt", args.datapath+"/amazon_test_tfidf.svm")
